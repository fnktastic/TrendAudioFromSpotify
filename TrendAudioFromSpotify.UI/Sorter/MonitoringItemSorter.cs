...
{
    public class MonitoringItemSorter : IComparer
    {
        private static readonly Regex numTextSplitRegex = new Regex(@"(?<=\D)(?=\d)|(?<=\d)(?=\D)", RegexOptions.Compiled);

        public int Compare(object o1, object o2)
        {
            var pl1 = o1 as MonitoringItem;
            var pl2 = o2 as MonitoringItem;

            string x = pl1.TargetPlaylistName;
            string y = pl2.TargetPlaylistName;
            x = x ?? "";
            y = y ?? "";
